 ...
 *
 *  - text over a surface axe declines to resolve. On first paint alone this
 *    page hands axe TWENTY of them, and they are not edge cases — the `<h1>`,
 *    the hero subtitle and description, the whole "Why it matters" aside, every
 *    Start-here card title and subtitle, all four audience buttons and the
 *    sticky nav links. They share one cause: this palette is built out of
 *    `color-mix(in oklab, …)` and layered `rgba()` panels, which
 *    `getComputedStyle` reports back unresolved.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This page uses opacity for
 *    de-emphasis more than any other mechanism: `.hs-chip-fam` and
 *    `.rec-chip-desc` at `.78`, `.pin-chip-fam` at `.8`, `.attack-venue` and
 *    `.cl-hero-sub` at `.85`, `.impl-glyph.impl-none` at `.6`,
 *    `.lamport-btn:disabled` at `.45`, and — the one that reaches furthest —
 *    `html[data-audience='…'] .lab-section:not([data-audience~='…'])` at
 *    `0.55`, which fades WHOLE SECTIONS the moment a reader picks an audience.
 *    The gate this replaces never picked one, so it never saw that state at all.
 *
 ...
 * the walk as a pair rather than fading the foreground alone. It also means
 * opacity COMPOUNDS: a `.rec-chip-desc` at `.78` inside a section faded to
 * `.55` is painted at `.43`, and only a walk that multiplies down the chain
 * reports the number a reader actually sees.
 *
 ...
 *    boxes are `.table-shell` and `.sticky-nav-inner` (`overflow-x: auto`), and
 *    `.hs-seg`, `.hs-bar`, `.size-track`, `.confidence-track` and `.skip-link`
 *    (`overflow: hidden`). `.hs-seg` is the load-bearing one: it is a flex
 *    segment whose width is animated from 0, its label is `white-space: nowrap`,
 *    and at phone width several segments are narrower than their own text, so
 *    the label really is clipped away rather than merely small. Content scrolled
 *    or clipped past those is not dimmed or partly drawn — it is absent from the
 ...
 *    behind it and fall through to white, inventing a failure.
 *
 ...
 *    has a real SVG figure — `.lat-svg`, the draggable 2D lattice — but every
 *    label belonging to it is HTML in the `.lat-readout` beside it, so the SVG
 *    itself carries no `<text>` and the sibling walk finds nothing to do today.
 *    It is kept because the alternative is a helper that silently mismeasures
 *    the first label anyone moves INTO that figure: an ancestor-only walk would
 *    composite a `<text>` onto the `<svg>`'s own transparent background rather
 *    than onto the `<polygon class="lat-fund">` drawn under it, and report a
 *    ratio nothing on screen has. The stroke-only guard inside it is live
 *    regardless: `.lat-axes` is two bare `<line>`s, and SVG's initial `fill` is
 *    black, so a walk that treated them as filled shapes would composite an
 *    opaque black rectangle across the whole figure.
 *
 ...
 * rule. `nontext.ts` measures the ones that carry a literal glyph; the ones
 * that are `content: ""` boxes coloured by `background` reach no oracle at all
 * and are hand-measured in the commit that introduced this gate.
 *
 * SECOND, `aria-hidden` subtrees, which BOTH oracles skip. See the `ariaHidden`
 * helper below for what this lab hides and why.
 */
 ...
 * uses. The genuinely short-lived styling here is `#copy-link`'s "Copied!"
 * swap and `#lamport-gen`'s "Generating…" label, both of which the drive waits
 * on explicitly rather than racing. The parameter is kept because the narrowed
 * form is what makes a short-lived state measurable at all: a class that lives
 * 500ms is shorter than a full axe pass, so a whole-page scan would catch it
 * only sometimes, and a sometimes-measured assertion is worse than none.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks six exhibits, the conformance table of NIST CAVP
     * vectors, and — the expensive part — hex output rendered one `<span>` per
     * byte pair in the byte-diff views, so a single 64-byte comparison is
     * hundreds of sibling elements all re-walking the same ancestors up to
     ...
     * that is `.table-shell` and `.sticky-nav-inner` (`overflow-x: auto`) and
     * `.hs-seg`, `.hs-bar`, `.size-track`, `.confidence-track` and
     * `.skip-link` (`overflow: hidden`).
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // exactly the shape that guard exists for — and so is this lab's own
      // `.lat-axes`, two bare `<line>`s that span the entire lattice figure.
      // The rule is the same one that would apply the moment a label moves
      // into that `<svg>`.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="gloss">` — the inline glossary `glossary.ts` wraps
      // around a defined term, all of which ship shut. The gate opens each by
      ...
      // the WCAG-sanctioned "visually hidden until focused" idiom, and this
      // page has TWO of them stacked: the shared header's `.cl-skip-link` parks
      // at `top: -3rem` and the lab's own `.skip-link` at `left: -10000px`,
      // each sliding in only on focus. Measuring the parked copies invents
      // failures for text that is not on screen; both focused renderings are
      // real states and the gate scans each of them explicitly instead.
      ...
      //
      // Kept, but NOT exercised here, and that is worth saying plainly rather
      // than implying coverage this page does not need. `clip: rect(0,0,0,0)`
      // and `clip-path: inset(50%)` are the two other visually-hidden idioms,
      // and neither `style.css` nor `extra.css` uses either — this lab's only
      // off-screen text is the two skip links, which park by POSITION and are
      // caught by the guard above. The guard stays because the clip idiom is
      // the one that is invisible to `checkVisibility()`: such an element keeps
      // a 1x1 rect and reports itself visible while the compositor draws zero
      // pixels of it, so the day an `.sr-only` arrives here — the usual way a
      // live region gets announced without being seen — a walk without this
      // would report a fabricated ratio for it on every single scan. It is
     ...
     *
     * What this lab hides is a closed set and every member of it is
     * decorative: the shared header's hamburger and GitHub marks, the
     * `.legend-dot` swatches in the lattice readout (each one sits immediately
     * beside the `b1` / `b2` / `shortest` label it colours), and the numeric
     * prefixes on the family tabs.
     *
     * Nothing on this page hides a VALUE. No digest, no signature, no leak
     * count, no reduction step, no ISD cost and no verdict is inside an
     ...
     * The 256-cell Lamport grid is the near miss: its cells carry no text at
     * all, and the grid is a `role="img"` whose `aria-label` is rewritten on
     * every paint, with `#lamport-grid-summary` repeating it as visible prose.
     */
     ...
     *
     * This lab's one SVG figure, `.lat-svg`, carries no `<text>`: every label
     * belonging to it is HTML in the `.lat-readout` beside it. The guard stays
     * because the usual way the mistake arrives is exactly
     * how this lab builds its DOM: `innerHTML` template literals. Interpolating
     ...
     *
     * It is a no-op today, and that was checked rather than assumed: `html`
     * declares no background, so the canvas value is taken from `body`, whose
     * `background` is a `var(--bg)` with no height pin. The guard stays because
     * this page is thirteen sections and many screens deep, so the day a
     * `height: 100%` appears on `html` or `body`, everything below the first
     * screen would report against a white page that does not exist, in a theme
     * whose surfaces are near-black.
     */
